...
export function ProcessSequence({ stages }: { stages: ProductionStage[] }) {
  ...
  return (
    <section
      id="production-line"
      ref={ref}
      className="shell scroll-mt-24 py-6 sm:py-10"
    >
      {/* The one dark surface on the site. The production line is the moment
          the brand is allowed to be theatrical; everything else stays light. */}
      <div className="glass-ink relative overflow-hidden rounded-2xl text-ground-100">
        <div
          aria-hidden
          className="absolute inset-0 bg-[radial-gradient(45rem_30rem_at_75%_0%,rgb(34_201_232/0.22),transparent_65%)]"
        />

        <div className="relative z-10 grid gap-8 px-6 py-10 sm:gap-10 sm:px-10 sm:py-16 lg:min-h-[38rem] lg:grid-cols-[1fr_1.15fr] lg:items-center lg:gap-16">
              ...
              >
                ...
                <span
                  className={cn(
                    ...
                    index === active ? "text-white" : "text-ink-400",
                  )}
                >
          ...
          <div className="relative lg:min-h-[26rem]">
            {stages.map((stage, index) => (
              ...
              >
                ...
                <p className="mt-4 max-w-xl leading-relaxed text-ground-300 sm:mt-6 sm:text-lg">
                  ...
                  <dl className="mt-8 grid max-w-lg grid-cols-2 gap-6">
                    {stage.metrics.map((metric) => (
                      <div key={metric.labelKey}>
                        <dt className="text-xs uppercase tracking-[0.18em] text-ink-400">
                          {metrics(metric.labelKey)}
                        </dt>
                        <dd className="mt-1 text-base text-aqua-300 tabular">
                          {metric.value}
                        </dd>
                      </div>
                    ))}
